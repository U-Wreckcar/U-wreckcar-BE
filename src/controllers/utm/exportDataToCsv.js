...
async function exportDataToCsv(req, res) {
    ...
    const sql_query = `SELECT DATE_FORMAT(utm.created_at, '%Y-%m-%d') as created_at, utm.utm_url as URL ,utm.utm_campaign_id as campaign_id,utm.utm_url, source.source_name as utm_source, medium.medium_name as utm_medium, utm.utm_campaign_name as campaign_name, utm.utm_term as campaign_term, utm.utm_content as campaign_content, utm.full_url as full_url, utm.shorten_url as Shorten_URL, utm.utm_memo as utm_memo FROM uwreckcar_db.Utms as utm LEFT join User_utm_sources as source on utm.user_utm_source_id = source.user_utm_source_id JOIN User_utm_mediums as medium on utm.user_utm_medium_id = medium.user_utm_medium_id ${sql_id_add} group by utm.utm_id `;
